...
namespace DBObjectsViewer
{
    public static class SQLRequests
    {
        ...
        {
            return "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' ORDER BY TABLE_NAME DESC;";
        }
        ...
        public static string TableIndexesRequest(string tableName)
        {
            return "select i.[name] as index_name, " +
                   ...
                   $"where t.is_ms_shipped <> 1 and index_id > 0 and t.[name] = {tableName} order by i.[name] DESC;";
        }
